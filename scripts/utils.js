"use strict";
function scrollOnLoad(x, y) {
    setTimeout(() => {
        window.scrollTo(x, y);
    }, 1);
}
function scrollWindowEvent(event) {
    window.scrollBy(-event.movementX, -event.movementY);
}
function findNodeInArea(x, y) {
    let nodeInArea = null;
...
function repaintCanvas(canvasCtx) {
    ...
    for (let node of graph.getNodes()) {
        ...
        drawNode(rectangle.x, rectangle.y, rectangle.width, rectangle.height, rectangle.text, highlightColor);
    }
    for (let nodeArrows of graph.getArrows()) {
        for (let arrow of nodeArrows) {
            drawArrow(canvasCtx, arrow.startX(), arrow.startY(), arrow.endX(), arrow.endY());
        }
    }
}
...
function connectNodesEvent(event) {
    ...
    if (firstNode) {
        ...
        if (foundNode === firstNode) {
            ...
            if (secondNode) {
                drawArrow(canvasCtx, firstNode.x + firstNode.width, firstNode.y + firstNode.height / 2, secondNode.x, secondNode.y + secondNode.height / 2);
...
function setupMoveNodeEvent(node) {
    isMovingNode = true;
    movingNode = node;
